...
**Phase 5 COMPLETE 2026-05-06 — Razorpay seva tiers (Pratham ₹11 / Anjali ₹51 / Bhakti ₹101 / Param ₹501) all working in test mode, diya seva panel + on-demand purchase from header, webhook handler with HMAC-SHA256 signature verification + idempotency, full e2e walkthrough verified across all four tiers + paywall + counter sync. Sub-phases: 5.1 schema migration (drop legacy `is_paid`, add UNIQUE on `payments.razorpay_order_id` for idempotent lookup); 5.2 Razorpay test-keys + webhook secret setup; 5.3 webhook handler with HMAC-SHA256 signature verification + payments refund columns (`refunded_at`, `razorpay_refund_id`) + `webhook_events` table (event_id PK + event_type + received_at + payload jsonb) + 5/5 smoke test scenarios pass + `scripts/smoke-test-webhook.ts` programmatic harness; 5.4 diya seva panel + counter + on-demand purchase via SevaTierPicker refactor + `/api/me` endpoint + chat response extension with `message_count` / `seva_balance` fields surfacing in both NDJSON meta frame and plain-JSON paths; 5.5 e2e walkthrough 7/7 scenarios pass + 7 polish fixes bundled (counter post-purchase sync via functional-updater, hi-default fall-through preserved on fresh visit, counter visual polish with `text-4xl sm:text-5xl text-devotional` ring-1 pill + edge-case dim at 0, `font-medium` on English message body for legibility parity with Devanagari, auto-grow textarea up to 6 lines + Enter/Shift+Enter, sticky language detection via `priorLang` inheritance for <3-word ambiguous turns inside an otherwise-Hindi conversation, flute label replacing "Messenger" text on Krishna message cards in-flow); 5.6 close-out. Schema additions: `payments.refunded_at` + `payments.razorpay_refund_id` columns, `webhook_events` table (idempotency keyed on `x-razorpay-event-id`), `payments.razorpay_order_id` UNIQUE constraint. Persona cache invariant: 10,065 tokens unchanged — no persona prompt edits in Phase 5; the new `Conversation language:` hint added in Phase 5.5 lives in the dynamic USER CONTEXT block, not the cached persona block, so cache_read sustains on turn 2+ as before. systemPrompt.ts unchanged (locked Phase 3). badWordFilter.ts unchanged. Disclaimer bar unchanged. Total Phase 5 spend ~₹200–300 (well within budget; mostly Razorpay test-mode round-trips + a handful of Sonnet chat turns during e2e walkthrough). Carry-forwards to Phase 6: real Razorpay-delivered webhook delivery test (needs public Vercel URL — smoke test programmatic only today), email-OTP auth (locked decision #14), refund auto-debit on `payments.refunded_at` (Phase 5.3 ships logging-only — refund triggers webhook event + DB write but does not auto-decrement `seva_balance`), real-device keyboard QA (Phase 4 deferred), chat history persistence on refresh (Phase 4 deferred — server-side context_summary intact, ChatUI React state empties), `src/lib/brand.ts` parameterization (rename batch flagged a third-pivot risk). NEXT: Phase 6 (mobile QA real device + Vercel deploy + Sentry + Plausible + privacy/terms + custom domain + KYC flip to live keys).**
