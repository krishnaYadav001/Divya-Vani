...
> **DIRECTION REVERSED — Dawn Aarti (2026-05-18), founder-approved.** Supersedes the Phase 8 (2026-05-17) *cinematic-dark temple-aarti* direction (which itself replaced the Phase 2.5 parchment/brass). The project now follows **Dawn Aarti** — a soft watercolor-pastel aesthetic, implemented from the [`design_handoff_dawn_aarti/`](design_handoff_dawn_aarti/README.md) Claude Design handoff. Rationale: founder-uploaded pastel Pichwai dawn fresco; mobile-first; Landing C selected after a three-option exploration. The six principles below now describe the CURRENT locked direction. (Decision log: [`docs/decisions.md`](docs/decisions.md), row 2026-05-18.)

1. **Commit to a BOLD aesthetic direction.** Divya Vani's direction is *Dawn Aarti* — a soft, watercolor pastel scene: warm mist/cream ground (`#faf7f2`), peach / rose / lavender / sky washes, gold-leaf accents, a vermillion sindoor seal, ink-brown text, with the founder's pastel Pichwai dawn fresco as the landing arch-portal image and drifting petals + sparkle motes as the living atmospheric ground on every page. Mobile-first. Every UI choice serves this direction; no SaaS conventions, no AI-default aesthetics.
2. **Typography:** UNCHANGED stack — Marcellus (English display / wordmark / UI labels) + Cormorant Garamond *italic* (English body) + Tiro Devanagari Hindi (ALL Devanagari). Hindi-first, English as secondary italic. Devanagari needs `line-height` 1.55–1.7 (default ~1.4 clips matras). NEVER introduce Inter / Roboto / Arial / Space Grotesk / system fonts / Noto Sans Devanagari (retired).
3. **Color:** semantic CSS variables in `src/app/globals.css`. The Phase 2.5 token NAMES (`--devotional` `--sacred` `--krishna` `--brass` `--parchment` `--peacock` + `*-dark`) are deliberately KEPT and REMAPPED to the Dawn pastel palette so components reskin globally — do not reintroduce dark/ink-ground values. New direct tokens: `--color-mist`/`-2`/`-3`, `--color-peach` / `--color-rose` / `--color-lavender` / `--color-sky` / `--color-buttermilk`, `--color-peacock` / `--color-peacock-deep`, `--color-gold-leaf`, `--color-vermillion`, `--color-ink` / `--color-ink-soft` / `--color-ink-faint` / `--color-ink-line`. Pastel washes are light: body text is `--color-ink` (`oklch(28% 0.035 30)`, AA on mist at 16px+); `ink-soft` / `ink-faint` are caption / italic ornament only — NEVER body text.
4. **Motion:** CSS-only (no Motion library). High-impact moments over scattered micro-interactions. Keep `fade-up`, `sonar` / `sonar-ring`, `dv-drift`, `dv-pulse`; add `drift-down` (petal fall), `twinkle` (sparkle motes), `float-y` (flute / feather idle), `breathe` (diya flame), `sonar-pulse` (mic listening). EVERY animation needs a `@media (prefers-reduced-motion: reduce)` off-switch. Stagger reveals via `[animation-delay:Xms] [animation-fill-mode:backwards]` (landing 0 / 180 / 360ms).
5. **Spatial:** asymmetry, overlap, generous negative space OR controlled density; at least one asymmetric grace note per page — the landing temple-arch portal (rounded `230px 230px 24px 24px` desktop / `140px 140px 14px 14px` mobile). Mobile-first: verify every screen at 360px wide.
6. **Atmosphere:** the `Atmosphere` component (`src/app/components/Atmosphere.tsx`, modes `hero` / `chat` / `corner` / `deep` / `mobile` / `distant`) is the z-0 ground on every page — drifting petals + sparkle motes, an optional fresco vignette (landing hero only), optional grain. Modes are reinterpreted (`hero` = full fresco + dense petals, `chat` = light petals + sparkles, `mobile` = reduced density). NEVER ship a flat untextured background; NEVER place the disclaimer/identity layer where the atmosphere reduces its legibility (Locked Decision #1).

**Banned across the project** (per skill + project brand): Inter / Roboto / Arial / system-ui / Space Grotesk / Noto Sans Devanagari fonts, purple-on-white gradients, generic SaaS card grids, carousels without narrative, flat untextured backgrounds.
